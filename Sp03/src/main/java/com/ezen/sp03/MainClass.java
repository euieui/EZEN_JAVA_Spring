...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		// 스프링 프레임워크에서는 클래스의 운용방식이 위와 같이 낳고, 정의된 클래스의 new 인스턴스를
		// 시작과 함께 미리 만들어서 필요할때 꺼내쓰는 방식으로 사용합니다.
		
		// 만들어진 인스턴스의 보관장소를 스프링 컨테이너 라고 부릅니다.
		// 현재 사용할 스프링 컨테이너 applicationContext.xml 입니다.
		
		// 스프링 컨테이너에 담겨있는 객체들을 Bean 이라고 부릅니다.
		...
		// 아직까지 일반 자바 프로젝트에서 사용하는 new WalkClass() 와 차이점은 없어 보입니다.
		// --- 도대체 왜이렇게 쓰냐 이거지?
		// 차이점을 굳이 써본다면 , new 인스턴스는 사용하면 사용할때마다 새로운 인스턴스가 생성되는 반면
		// getBean()은 싱글턴 방식처럼 하나의 생성된 객체가 계속 사용된다는 점이 다릅니다.
		// getBean()으로 같은 클래스의 서로 다른 인스턴스를 두개 사용해야 한다면, 다른 id 값으로 Bean을 
		// 두개 추가하여 사용합니다.
		// <bean class="com.ezen.spcon.WalkClass" id="cWalk2"></bean> ---- xml 에서 생성하라는 말이군
